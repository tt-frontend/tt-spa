import StarIcon from './svg/star.svg?react';
import CheckIcon from './svg/check.svg?react';
import HistoryIcon from './svg/history.svg?react';
import SwitchIcon from './svg/switch.svg?react';
import InfoIcon from './svg/info.svg?react';
import StopIcon from './svg/stop.svg?react';
import WarningIcon from './svg/warning.svg?react';
import ColdWaterSupplyIcon from './svg/coldWaterSupply.svg?react';
import HotWaterSupplyIcon from './svg/hotWaterSupply.svg?react';
import AllResourcesIcon from './svg/allResources.svg?react';
import ElectricityIcon from './svg/electricity.svg?react';
import HeatIcon from './svg/heat.svg?react';
import DeviceIcon from './svg/device.svg?react';
import StatisticIcon from './svg/statistic.svg?react';
import ReportsIcon from './svg/reports.svg?react';
import TasksIcon from './svg/tasks.svg?react';
import ObjectsIcon from './svg/objects.svg?react';
import CompanyProfileIcon from './svg/companyProfile.svg?react';
import DocumentIcon from './svg/document.svg?react';
import SettingsIcon from './svg/settings.svg?react';
import UserIcon from './svg/user.svg?react';
import PencilIcon from './svg/pencil.svg?react';
import TrashIcon from './svg/trash.svg?react';
import UploadIcon from './svg/upload.svg?react';
import DateIcon from './svg/date.svg?react';
import DownloadIcon from './svg/download.svg?react';
import ExitIcon from './svg/exit.svg?react';
import TimerIcon from './svg/timer.svg?react';
import OkIcon from './svg/ok.svg?react';
import CloseIcon from './svg/close.svg?react';
import MapIcon from './svg/map.svg?react';
import NumberIcon from './svg/number.svg?react';
import CalendarIcon from './svg/calendar.svg?react';
import MoreIcon from './svg/more.svg?react';
import CalculatorIcon from './svg/calculator.svg?react';
import ChoiceIcon from './svg/choice.svg?react';
import FinishIcon from './svg/finish.svg?react';
import ChevronIcon from './svg/chevron.svg?react';
import PersonIcon from './svg/person.svg?react';
import PureResourceIcon from './svg/pureResource.svg?react';
import NotRegisteredIcon from './svg/notRegistered.svg?react';
import PreparedForDeliveryIcon from './svg/preparedForDelivery.svg?react';
import PendingApprovalIcon from './svg/pendingApproval.svg?react';
import RegisteredIcon from './svg/registered.svg?react';
import TelegramIcon from './svg/telegram.svg?react';
import SearchIcon from './svg/search.svg?react';
import ListIcon from './svg/list.svg?react';
import CellsIcon from './svg/cells.svg?react';
import PauseIcon from './svg/pause.svg?react';
import SputnikIcon from './svg/sputnik.svg?react';
import DubbedIcon from './svg/dubbed.svg?react';
import EyeIcon from './svg/eye.svg?react';
import CityIcon from './svg/city.svg?react';
import BagIcon from './svg/bag.svg?react';
import PointIcon from './svg/point.svg?react';
import KeysIcon from './svg/keys.svg?react';
import CrownIcon from './svg/crown.svg?react';
import ArrowRightLongIcon from './svg/arrowRightLong.svg?react';
import ClosedEyeIcon from './svg/closedEye.svg?react';
import CitySmallIcon from './svg/citySmall.svg?react';
import NoConnectionIcon from './svg/noConnection.svg?react';
import PipeIcon from './svg/pipe.svg?react';
import BriefcaseIcon from './svg/briefcase.svg?react';
import HouseIcon from './svg/house.svg?react';
import IncorrectConfigurationIcon from './svg/incorrectConfiguration.svg?react';
import ActiveFiltersIcon from './svg/activeFilters.svg?react';
import ChevronBoldIcon from './svg/chevronBold.svg?react';
import CheckConnection from './svg/checkConnection.svg?react';
import SortingIcon from './svg/sorting.svg?react';
import PlusIcon from './svg/plus.svg?react';
import ApartmentIcon from './svg/apartment.svg?react';
import PencilBigIcon from './svg/pencilBig.svg?react';
import WrenchIcon from './svg/wrench.svg?react';
import CheckMark from './svg/checkMark.svg?react';
import PlusSmallIcon from './svg/plusSmall.svg?react';
import FileIcon from './svg/file.svg?react';
import DownloadBlueIcon from './svg/downloadBlue.svg?react';
import DocumentLargeIcon from './svg/documentLarge.svg?react';
import ErcIcon from './svg/erc.svg?react';
import GosUslugiIcon from './svg/gosuslugi.svg?react';
import BankIcon from './svg/bank.svg?react';
import ArchiveIcon from './svg/archive.svg?react';
import ArrowIconTop from './svg/arrow.svg?react';
import ArrowBottom from './svg/arrowBottom.svg?react';
import FilterIcon from './svg/filterIcon.svg?react';
import FilterIconBlue from './svg/filterIconBlue.svg?react';
import FilterIconTransparent from './svg/filterIconTransparent.svg?react';
import MapPaperIcon from './svg/mapPaper.svg?react';
import TemperatureIcon from './svg/temperature.svg?react';
import CheckedBoxIcon from './svg/checkedBox.svg?react';
import AscendingSortIcon from './svg/ascendingSort.svg?react';
import DescendingSortIcon from './svg/descendingSort.svg?react';
import DocumentBlueIcon from './svg/documentBlue.svg?react';
import ArrowTopIcon from './svg/arrowTop.svg?react';
import DoubleChevronLeft from './svg/doubleChevronLeft.svg?react';
import ConnectionIcon from './svg/connection.svg?react';
import DocumentBoldIcon from './svg/documentBold.svg?react';
import LoadingBlueIcon from './svg/loadingBlue.svg?react';
import CheckGreenIcon from './svg/checkGreen.svg?react';
import TrashIconGrey from './svg/trashGrey.svg?react';
import ChevronTopActive from './svg/chevronTopActive.svg?react';
import ChevronTopDanger from './svg/chevronTopDanger.svg?react';
import ExportIcon from './svg/export.svg?react';
import DownloadGrayIcon from './svg/downloadGray.svg?react';
import DocumentBigIcon from './svg/documentBig.svg?react';
import DeviceBigIcon from './svg/deviceBig.svg?react';
import CheckCircleIcon from './svg/checkCircle.svg?react';
import QuestionMarkCircleIcon from './svg/questionMarkCircle.svg?react';
import XCircleIcon from './svg/xCircle.svg?react';
import ResetIcon from './svg/reset.svg?react';
import CitySmallGrayIcon from './svg/citySmallGray.svg?react';
import IndividualDeviceSmallIcon from './svg/individualDeviceSmall.svg?react';
import MeasurementSmallIcon from './svg/measurementSmall.svg?react';
import CurrentAnalyticsIcon from './svg/currentAnalytics.svg?react';
import MagnifierIcon from './svg/magnifier.svg?react';
import StopOrangeIcon from './svg/stopOrange.svg?react';
import OtherSourceIcon from './svg/otherSource.svg?react';
import ChevronActiveIcon from './svg/chevronActive.svg?react';
import CloseBlueIcon from './svg/closeBlue.svg?react';
import ReloadBlueIcon from './svg/reloadBlue.svg?react';
import WarningYellowIcon from './svg/warningYellow.svg?react';
import StopRedIcon from './svg/stopRed.svg?react';
import ClipIcon from './svg/clip.svg?react';
import CloseDarkIcon from './svg/closeDark.svg?react';
import MainIcon from './svg/mainIcon.svg?react';

export {
  FinishIcon,
  ChoiceIcon,
  MoreIcon,
  StopIcon,
  CalculatorIcon,
  CalendarIcon,
  NumberIcon,
  MapIcon,
  CloseIcon,
  OkIcon,
  StarIcon,
  CheckIcon,
  HistoryIcon,
  SwitchIcon,
  InfoIcon,
  WarningIcon,
  ColdWaterSupplyIcon,
  HotWaterSupplyIcon,
  ElectricityIcon,
  HeatIcon,
  AllResourcesIcon,
  DeviceIcon,
  StatisticIcon,
  ReportsIcon,
  TasksIcon,
  ObjectsIcon,
  CompanyProfileIcon,
  SettingsIcon,
  UserIcon,
  DocumentIcon,
  PencilIcon,
  TrashIcon,
  TimerIcon,
  UploadIcon,
  DateIcon,
  DownloadIcon,
  ExitIcon,
  ChevronIcon,
  PersonIcon,
  PureResourceIcon,
  NotRegisteredIcon,
  PreparedForDeliveryIcon,
  PendingApprovalIcon,
  RegisteredIcon,
  TelegramIcon,
  CellsIcon,
  PauseIcon,
  ListIcon,
  SearchIcon,
  SputnikIcon,
  DubbedIcon,
  EyeIcon,
  CityIcon,
  BagIcon,
  PointIcon,
  KeysIcon,
  CrownIcon,
  ArrowRightLongIcon,
  ClosedEyeIcon,
  CitySmallIcon,
  NoConnectionIcon,
  PipeIcon,
  HouseIcon,
  BriefcaseIcon,
  IncorrectConfigurationIcon,
  ActiveFiltersIcon,
  CheckConnection,
  ChevronBoldIcon,
  SortingIcon,
  PlusIcon,
  ApartmentIcon,
  PencilBigIcon,
  WrenchIcon,
  CheckMark,
  PlusSmallIcon,
  FileIcon,
  DownloadBlueIcon,
  DocumentLargeIcon,
  ErcIcon,
  GosUslugiIcon,
  BankIcon,
  ArchiveIcon,
  ArrowIconTop,
  ArrowBottom,
  FilterIcon,
  FilterIconBlue,
  FilterIconTransparent,
  MapPaperIcon,
  TemperatureIcon,
  CheckedBoxIcon,
  AscendingSortIcon,
  DescendingSortIcon,
  DocumentBlueIcon,
  ArrowTopIcon,
  DoubleChevronLeft,
  ConnectionIcon,
  DocumentBoldIcon,
  LoadingBlueIcon,
  CheckGreenIcon,
  TrashIconGrey,
  ChevronTopActive,
  ChevronTopDanger,
  ExportIcon,
  DownloadGrayIcon,
  DocumentBigIcon,
  DeviceBigIcon,
  CheckCircleIcon,
  QuestionMarkCircleIcon,
  XCircleIcon,
  ResetIcon,
  CitySmallGrayIcon,
  IndividualDeviceSmallIcon,
  MeasurementSmallIcon,
  CurrentAnalyticsIcon,
  MagnifierIcon,
  StopOrangeIcon,
  OtherSourceIcon,
  ChevronActiveIcon,
  CloseBlueIcon,
  ReloadBlueIcon,
  WarningYellowIcon,
  StopRedIcon,
  ClipIcon,
  CloseDarkIcon,
  MainIcon,
};
